    ...
    {foreach $elements as $key => $profileField}
        ...
        {if {array_key_exists array=$profileField key="category"}}
            {if $profileField.category != $lastCategory}
                {if $lastCategory != ''}
                    {if isset($pPreferences->config.cat_texts["{$profileField.catid}_posttext"])}
                        {$pPreferences->create_html($pPreferences->config.cat_texts["{$profileField.catid}_posttext"])}
                    {/if} 
                    </div></div>
                {/if}
                ...
                <div class="card admidio-field-group">
                    ...
                    <div class="card-body">
                        ...
                        {if isset($pPreferences->config.cat_texts["{$profileField.catid}_pretext"])}
                            {$pPreferences->create_html($pPreferences->config.cat_texts["{$profileField.catid}_pretext"])}<br><br>
                        {/if}
            {/if}
