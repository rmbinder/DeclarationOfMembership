...
use Plugins\DeclarationOfMembership\classes\Service\FormValidation;
...
try {
    ...
    foreach ($gProfileFields->getProfileFields() as $field) {
        ...
        if ($showField) {
            if (isset($_POST[$postId])) {
                ...
                // Ausgabe der Fehlermeldung je nach Datentyp (--> sind diese Fehlermeldungen eigentlich notwendig?)
                if (! $returnCode) {
                    switch ($field->getValue('usf_type')) {
                        case 'CHECKBOX':
                            throw new Exception($gL10n->get('SYS_FIELD_INVALID_INPUT', array(
                                $field->getValue('usf_name')
                            )));
                            // => EXIT
                            break;
                        case 'DATE':
                            throw new Exception($gL10n->get('SYS_DATE_INVALID', array(
                                $field->getValue('usf_name'),
                            ...
                            break;
                        case 'EMAIL':
                            throw new Exception($gL10n->get('SYS_EMAIL_INVALID', array(
                                $field->getValue('usf_name')
                            )));
                            // => EXIT
                            break;
                        case 'NUMBER':
                        case 'DECIMAL':
                            throw new Exception($gL10n->get('SYS_FIELD_NUMERIC', array(
                                $field->getValue('usf_name')
                            )));
                            // => EXIT
                            break;
                        case 'PHONE':
                            throw new Exception($gL10n->get('SYS_PHONE_INVALID_CHAR', array(
                                $field->getValue('usf_name')
                            )));
                            // => EXIT
                            break;
                        case 'URL':
                            throw new Exception($gL10n->get('SYS_URL_INVALID_CHAR', array(
                                $field->getValue('usf_name')
                            )));
                            // => EXIT
                            break;
                ...
                if ($field->getValue('usf_type') === 'CHECKBOX') {
                    ...
                    throw new Exception($gL10n->get('SYS_FIELD_EMPTY', array(
                        $field->getValue('usf_name')
                    )));
                    // => EXIT
                }
    ...
    if ($pPreferences->config['usr_login_name']['displayed']) {
        if ($_POST['usr_login_name'] !== $user->getValue('usr_login_name')) {
            ...
            if (! $user->setValue('usr_login_name', $_POST['usr_login_name'])) {
                throw new Exception($gL10n->get('SYS_FIELD_INVALID_CHAR', array(
                    'SYS_USERNAME'
                )));
                // => EXIT
            }
    ...
    // und 
    // 2. wenn entweder eine "Absender E-Mail" oder eine "Administrator E-Mail" definiert ist
    if ($pPreferences->config['emailnotification']['access_to_module'] && ((strlen($gSettingsManager->getString('mail_sendmail_address')) > 0) || (strlen($gCurrentOrganization->getValue('org_email_administrator')) > 0))) {
        $senderEmail = (strlen($gSettingsManager->getString('mail_sendmail_address')) > 0) ? $gSettingsManager->getString('mail_sendmail_address') : $gCurrentOrganization->getValue('org_email_administrator');
    ...
    if ($pPreferences->config['options']['kiosk_mode']) {
        ...
        $page = PagePresenter::withHtmlIDAndHeadline('plg-declaration-of-membership-exit-or-continue');
        $page->setHeadline($headline);

        // create html form
        $form = new FormPresenter('exit-or-continue-form', '../templates/save.exit.or.continue.plugin.declarationofmembership.tpl', '', $page);
