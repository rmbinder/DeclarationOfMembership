...
if ($showOption == 'refresh') {
    ...
    $page->addJavascript('
        ...
        $("#tabs-common").attr("class", "tab-pane active");', true);
}
        ...
        $.post({
            ...
            success: function(data) {
                if (data === "success" || data === "refresh") {
                    ...
                    if(data === "refresh") {
                        window.location.replace("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences.php', array(
    'show_option' => 'refresh'
)) . '");
                    } 
                } else {
...
$formDisplayedFields = new HtmlForm('displayed_fields_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
...
$formDisplayedFields->addMultilineTextInput('main_pretext', $gL10n->get('PLG_DECLARATION_OF_MEMBERSHIP_PRETEXT'), (isset($pPreferences->config['main_texts']['main_pretext']) ? $pPreferences->config['main_texts']['main_pretext'] : ''), 3, array(
...
foreach ($gProfileFields->getProfileFields() as $field) {
    ...
    if ($category !== $cat_name) {
        ...
        $formDisplayedFields->addDescription($noteText);
...
$formDisplayedFields->addSubmitButton('btn_save_configurations', $gL10n->get('SYS_SAVE'), array(
    'icon' => 'bi-check-lg',
    'class' => ' offset-sm-3'
));
...
$formRequiredFields = new HtmlForm('required_fields_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
...
$formRequiredFields->addSubmitButton('btn_save_configurations', $gL10n->get('SYS_SAVE'), array(
    'icon' => 'bi-check-lg',
    'class' => ' offset-sm-3'
));
...
$formOptions = new HtmlForm('options_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
    'form' => 'options'
)), $page, array(
    'class' => 'form-preferences'
));
...
$formOptions->addSubmitButton('btn_save_configurations', $gL10n->get('SYS_SAVE'), array(
    'icon' => 'bi-check-lg',
    'class' => ' offset-sm-3'
));
...
$formEmailnotification = new HtmlForm('emailnotification_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
...
$formEmailnotification->addSelectBox('enable_emailnotification', $gL10n->get('PLG_DECLARATION_OF_MEMBERSHIP_ACCESS_TO_MODULE_AUTOREPLYMAIL'), $selectBoxEntries, array(
    'defaultValue' => $pPreferences->config['emailnotification']['access_to_module'],
    'showContextDependentFirstEntry' => false,
...
if ($pPreferences->config['emailnotification']['access_to_module']) {
    ...
    $formEmailnotification->addInput('msg_subject', $gL10n->get('SYS_SUBJECT'), $pPreferences->config['emailnotification']['msg_subject'], array(
        'maxLength' => 77,
        'helpTextId' => 'PLG_DECLARATION_OF_MEMBERSHIP_AUTOREPLYMAIL_INFO',
        'property' => HtmlForm::FIELD_REQUIRED
    ));
    $formEmailnotification->addEditor('msg_body', '', $pPreferences->config['emailnotification']['msg_body'], array(
        'property' => HtmlForm::FIELD_REQUIRED
    ));
}

$formEmailnotification->addSubmitButton('btn_save_configurations', $gL10n->get('SYS_SAVE'), array(
    'icon' => 'bi-check-lg',
    'class' => ' offset-sm-3'
));
...
$formUninstallation->addSubmitButton('btn_save_uninstallation', $gL10n->get('PLG_DECLARATION_OF_MEMBERSHIP_UNINSTALLATION'), array(
    'icon' => 'bi-trash',
    'class' => 'offset-sm-3'
));
...
$formAccessPreferences = new HtmlForm('access_preferences_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
    'form' => 'access_preferences'
)), $page, array(
    'class' => 'form-preferences'
));
...
$formAccessPreferences->addSubmitButton('btn_save_access_preferences', $gL10n->get('SYS_SAVE'), array(
    'icon' => 'bi-check-lg',
    'class' => ' offset-sm-3'
));
